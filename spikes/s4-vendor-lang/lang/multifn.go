...
func NewMultiFn(name string, dispatchFn IFn, defaultDispatchVal any, hierarchy IRef) *MultiFn {
	...
	// cljgo S4 surgery: registerWellKnownMethods / IsAutoRegisteredMethod
	// deleted — they seeded a *Class print-method (pkgmap host-class glue
	// from class.go), which does not exist in the severed runtime.
	return mf
}
