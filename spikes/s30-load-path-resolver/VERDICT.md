...
The blocker is elsewhere, and S30 puts it in sharp relief: **only the
...
> hijacking a dependency's sibling (S30 §2, measured).
>
...
> classpath semantics, which does allow such shadowing (S30 §3, oracled
> against Clojure 1.12.5).
>
> Among load-path roots, first-wins, silently — matching classpath
> semantics exactly (S30 §4, oracled). A shadowing *diagnostic* is
...
> silently contribute source to a `cljgo build` artifact (S30 §5).
>
...
> construction — verified end-to-end, byte-identical output (S30 §1).
...
divergence of the ADR 0002/0007 class and is out of scope for S30, but it
