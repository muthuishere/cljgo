# Spike S28 — What is the smallest fetch + cache + lock that makes `(dep b …)` reproducible?

Opened 2026-07-22. Feeds **ADR 0048** decisions **1** (cache) and **3**
(lockfile). Runs in parallel with S25 (load path), S26 (version conflict),
S27 (purity/manifest).
...
S25 owns *where a resolved dep is found from* (the load path). **S28 owns
what makes the thing at that path trustworthy and identical everywhere.**
The division: S25 answers "how does `a.core` resolve"; S28 answers "is the
...
- **Decision 6 (purity) is UNRESOLVED**, but names `go-require` / `c-link` /
  `ffi` propagation as the open question. Whatever S27 concludes, resolution
  ...
  even though S28 does not decide policy on them.
...
Anything less closes S28 **no** for the mechanism tested, and ADR 0048
...
2. **Vendor escape hatch.** Project-local `vendor/` for air-gapped builds.
   Does it interact cleanly with S25's load path ordering (ADR 0048 §2), or
...
*conflict* policy is S26's, not S28's. Purity *policy* is S27's; S28 only
