# Spike S28 verdict — SHA pins identity, tree hash pins content; the lock needs both, plus transitivity and impurity
...
Interaction with **ADR 0048 decision 2** (S25's load path): vendoring does
...
S25 a different base directory for the same dep, in the same lock order.
...
code-first surface — that is **S27's question**, and S27 may conclude it
cannot. What S28 establishes is narrower and still useful: *given* a
...
reads at all beyond the ones it re-verifies. If S27 fails, the manifest can
be emitted at publish time or — the fallback S27 should cost — reconstructed
into the lock by the *publisher's* build, so consumers still read data.

## 8. Version conflict (S26's, not this spike's)

The prototype refuses rather than picks:

```
dependency conflict: "x" required at ref "a" and "b"
  (S26 owns the policy; S28 refuses to pick)
```

Recorded here only to note the boundary held: nothing in the cache or lock
design presumes a resolution algorithm, so S26 can choose hard-error (ADR
...
> cannot detect a modified cache entry or a lying mirror [S28 §1].
>
...
> rather than invisible [S28 §5].
>
> Deps are emitted sorted by name and maps with sorted keys, so two machines
> produce byte-identical lockfiles [S28 §E1c, `cmp` exit 0].
...
| resolved Go-module *versions* merged at lock time | this is decision 6's open question and S26's call; recording each dep's *declared* `go-require` keeps the lock honest without pre-deciding the merge |
...
- The **shape** of `:impure` is provisional. S28 proves the lock *can* carry
  ...
  the lock alone**. The *policy* (refuse? warn? propagate?) is S27's.
- Whether `cljgo.manifest.edn` can exist at all is **S27's**. If it cannot,
