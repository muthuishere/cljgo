# S29 VERDICT — one transitive walk yields both purity granularities
...
2. runs N independent **taint predicates** over every captured `CompiledNS`
   ...
   abstract `javaStubPredicate` standing in for **S30**'s `uses-java?`, and a
...
1. **Detection mechanism is real (Go-interop).** `require-go` surfaces in the
   ...
   AST ops in `pkg/` today** (S7/S21/S27 + future ADR 0011/0044); the pluggable
...
4. **Classification keys a hard-error-not-nil (item 5, confirmed not
   ...
   *analyze* — e.g. `(java.util.UUID/randomUUID)`, whose namespace resolves to
   ...
   `nil`. S29 does not fix §6a; it confirms the validator produces the
...
## Empirical Java observations (handed to S30, not S29's verdict)

Recorded in passing (`cljgo run` on scratch fixtures), because they shape the
S30 predicate and ADR 0050 decision 4:
...
- `(.getBytes "hello")` → **runtime error** (`no method getBytes on string`,
  ...
  Go dot-method** `(.ToUpper x)` at the AST level. This is the S30 crux: an
...
- **Decision 4 (per-ns use gate + Java third class): adopt, with a caveat named
  by S29 and owned by S30.** Per-ns gating from the same map works. The Java
  ...
  dot-form instance methods are Go/Java-ambiguous at the AST level, so the S30
...
```
go run ./spikes/s29-transitive-purity-validator            # all fixtures
go run ./spikes/s29-transitive-purity-validator -fixture go-buried
```
