// Spike S29 driver — ONE transitive walk that classifies every reachable
...
// adapted from pkg/emit/emit.go:eachChild (unexported there) — S29 owns its
// own copy per the spike rules.
//
//	go run ./spikes/s29-transitive-purity-validator            # all fixtures
//	go run ./spikes/s29-transitive-purity-validator -fixture mixed
package main
...
// ---- the java predicate (ABSTRACT — S30 owns the real primitive) ----------

// javaStubPredicate stands in for S30's uses-java?(ns). S29 does not decide
...
func javaStubPredicate(usesJava map[string]bool) Predicate {
	return func(ns *emit.CompiledNS) []Finding {
		...
		return []Finding{{Class: ClassJava, NS: name, Path: ns.Path, Line: 1,
			Detail: "flagged by external uses-java? predicate (S30)"}}
	}
}

// ffiPredicate is the future slot (ADR 0011/0044/S27). No ffi/deflib AST op
...
// per Op. S29 keeps its own copy per the spike read-only rule.
...
type fixture struct {
	...
	usesJava map[string]bool // stub S30 predicate input
...
func main() {
	...
	base := filepath.Join("spikes", "s29-transitive-purity-validator", "fixtures")
