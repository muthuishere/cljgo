# Spike S29 — One transitive walk that classifies every reachable namespace by purity
...
host interop — is owned by a dedicated spike **S30** and is explicitly OUT of
S29's scope. S29 treats Java taint as an **abstract third class** supplied by an
external, pluggable predicate `uses-java?(ns)`; S30 proves that predicate. S29's
...
Empirical Java observations seen in passing are recorded (they inform S30), but
S29's verdict does not rest on distinguishing a Java method-call from a Go one.
...
- `pkg/emit/module.go` `CompileProgram(srcPath)` already **walks the transitive
  ...
  This is the traversal S29 must reuse rather than reinvent.
...
- `require-go` is the only concrete Go-dep marker in `pkg/` today. `ffi` /
  `deflib` / `c-link` are **not yet AST ops** (they are S7/S21/S27 spike-level
...
4. **java-abstract** — a fixture whose `jav.leaf` is flagged by an injected
   stub `uses-java?` predicate (standing in for S30). The walk surfaces it as a
...
need separate traversals, or a pure lib shows spurious taint — closes S29 **no**
...
3. What did the Java fixture actually DO when analyzed by cljgo (for S30)?
