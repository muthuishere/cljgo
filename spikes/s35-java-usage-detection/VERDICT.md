...
- **`build` / `publish --go`:** the Go emitter/compiler rejects an
  ...
  produce a broken Go binary. (S35 §1 shows the interpreter analog: `no
  method X on Y`.)
...
## 2. Why the dot-form is irreducible — the crux (S34's Q1–Q3, answered)
...
>    S34). **Do not flag bare `(.method obj)`** (on the JVM it is a Java
...
>    These already hard-error at analysis with `file:line` (S34, S35 §1);
...
> runtime receiver (S35 §2, measured). Positively naming it Java is
