// Spike S26 driver — exercises pkg/emit.SynthGoMod with MERGED require sets,
...
//
//	go run ./spikes/s26-impure-dep-go-modules -case flatten
//	go run ./spikes/s26-impure-dep-go-modules -case overwrite
//	go run ./spikes/s26-impure-dep-go-modules -case conflict
//	go run ./spikes/s26-impure-dep-go-modules -case manifest
package main
...
func caseManifest() error {
	...
	// S26 does NOT invent a manifest format. Spike S28 already prototyped the
	// lockfile (`build.lock.edn`) with a per-dep `:impure {:go-require [...]}`
	// block — see spikes/s28-dep-fetch-cache-lock/prototype/resolve.go:148,181.
...
// pkg/reader (pure data, no eval), exactly as S28's prototype does. The point
