# Spike S26 verdict — impurity does not merge cleanly, and it already breaks dual-mode parity
...
| F | transitive discovery from S28's lock schema, 3 merge policies | `results/F-lock-discovery.txt` |
...
written (S25). No rewrite-an-existing-`go.mod` design is needed, and the
...
consumer's own `./build.cljgo` is ever read** (S25). A dependency's
...
**Do not invent a manifest format for that. S28 already has the answer.** Its lock
prototype writes a per-dep `:impure {:go-require […] :c-link […] :ffi […]}`
block (`spikes/s28-dep-fetch-cache-lock/prototype/resolve.go:148,181`). A
...
```
-- resolve: read build.lock.edn (S28 schema); never evaluate a build fn --
...
dep's own `:impure` data came from in the first place; that is S27's
question (can ADR 0021's code-first `build.cljgo` emit such data at publish
time). S26 confirms the *consumption* side works; the *production* side is
unresolved.
...
### ADR 0048 decision 5 — the lock is the answer; adopt S28's schema by reference
...
`:impure :go-require` block (S28's schema), read as data with `pkg/reader`.
Two constraints S26 adds:
...
2. Decision 5 is only half-answered. S27 still owns whether a dep can
...
3. **Designing a fresh manifest format for decision 5.** S28's lock schema
...
| 5. transitive discovery without eval | yes, via S28's lock schema | `F` |
...
- **S27** must answer the production side of §2.6 and owns the cgo /
