// S27 exp4 — a RESOLVE-TIME impurity check.
//
		...
		for _, c := range m.CLink {
			if needCross {
				findings = append(findings, finding{m.Name, "cgo",
					fmt.Sprintf("c-link %q forces CGO_ENABLED=1; this project requires cross-compilation, which cgo against a third-party system library cannot do (S27 exp2: zig-cc supplies libc, not the library's headers)", c.PkgConfig),
