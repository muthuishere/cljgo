...
This predates S27 and belongs to the `require-go` interpreter path
...
error: cljgo resolve refused the dependency graph (4 problem(s)), before fetching or building anything.
  ...
  fastjson-cgo [:cgo]
    ...
    which cgo against a third-party system library cannot do (S27 exp2: zig-cc supplies
...
```
cd spikes/s27-impure-dep-ffi-cgo
