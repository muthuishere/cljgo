# Spike S31 — Can the interpreter tell an UNLINKED third-party `require-go` member from a legitimate `nil`?
...
S26 / S27, which **already measured** that the divergence exists — this
...
member-access forms through the interpreter (S26 §2.1, S27 §1.3).
...
Throwaway prototype. Per ADR 0027 + S25's sanctioned method, where
