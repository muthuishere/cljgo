# Spike S31 verdict — the unlinked-vs-nil distinction is reliably detectable; the fix is one mode-aware predicate
...
- **miss + `isThirdPartyGoPath` true** → a member of a domain-dotted
  ...
  interpreter binary (only stdlib + cljgo-own are in the registry; S26
  §2.1, S27 §1.3). This is the **only** path that produces the silent nil,
  and it is case **(a)**.
...
> (design/05 §1)"` — instead of returning nil (S31, MEASURED). The check is
...
git apply spikes/s31-unlinked-goref-detection/prototype.patch
go build -o /tmp/cljgo-s31 ./cmd/cljgo                      # patched
F=spikes/s31-unlinked-goref-detection/fixtures
