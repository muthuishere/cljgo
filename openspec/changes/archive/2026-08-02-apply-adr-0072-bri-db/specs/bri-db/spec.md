> Namespace reconciled before archiving: ADR 0072 shipped as `bri.db`, which
> ADR 0102 later renamed in place to `cljg.data.cast` (`core/cljg/data_cast.cljg`,
> `pkg/bri/bri.go` Specs row, shims still under `pkg/bri/db`). The requirements
> below name the namespace that exists today.
