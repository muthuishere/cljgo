...
lives only in `spikes/s28-.../prototype/`. Per ADR 0027 spike code never merges —
...
- **`pkg/eval/libload.go:65-100`** — `ResolveLibPath`; roots built at `:75-86`,
  ...
  are all-new appends to `roots` at the `:86`/`:88` seam — exactly where S25's
...
- **`pkg/reader`** — `ReadString`, `Reader.ReadAll()` parse EDN into cljgo data;
  `evalLibFile` (`libload.go:106-133`) and S27 exp4 are working templates.

## Goals / Non-Goals

**Goals:**
- A `pkg/deps` package implementing decisions 1 (cache), 3 (lock), 4/5/6
  (version conflict, transitive-from-lock, purity), re-authored from S28/S26/S27
...
1. **`pkg/deps` package, re-authored from S28.** Port `cas.go` (`CacheRoot()`
   ...
   EDN emitter → byte-identical lockfiles). **Replace S28's throwaway
...
4. **Version conflict merged BEFORE the go.mod write.** cljgo detects duplicate
   Go-module requires at different versions in its own layer (port S26's merge
...
6. **Purity gate, default-deny, ffi/cgo split.** Port S27 exp4's resolve-time
...
- **Real git in tests** → hermetic: tests create throwaway local git repos in
  `t.TempDir()` and use `file://` transports (S28 did exactly this; darwin-only
  `flock` is already how S28 ran). No network in the test suite.
...
- Exact `build.cljgo` surface for the **consumer-side version override** and the
  **capability `:allow` set** — S27 used `build.cljgo.policy.edn`/`:allow`; fold
