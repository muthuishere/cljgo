...
- [x] 2.2 `validate(pom)` — detect and `G5011` on `${property}`, `<dependencyManagement>`, `<parent>`, ranges, `-SNAPSHOT`, `<profiles>`, `<classifier>`, non-jar packaging. Name the feature AND the coordinate.
