...
- [x] 1.1 `pkg/bri/db.go`: `installDBShims` interning the private primitives —
...
- [x] 1.2 Handle model: `*dbHandle` wraps `*sql.DB`, `*txHandle` wraps
...
- [x] 1.3 Rows→maps: `snake_case` column → `kebab-case` keyword key.

## 2. The Clojure API

- [x] 2.1 `core/bri/db.cljg` (ns `bri.db`): `connect` `close!` `query` `one`
  `one!` `exec!` `insert!` `update!` `delete!` `tx` `with-rollback` `migrate!`
  `migrate-status`. `one!` throws `ex-info` tagged `:bri.db/not-found`.
- [x] 2.2 Embed the source in `core/bri.go` (`//go:embed bri/db.cljg` →
  `BriDBSource`).

## 3. Wire into the AOT mechanism

- [x] 3.1 Add the `bri.db` Spec to `pkg/bri.Specs()` (after bri.config; nothing
  bri depends on it at load time) with `install: installDBShims`.
- [x] 3.2 Add `modernc.org/sqlite` + `pgx/v5` to the root `go.mod`; regenerate
  `pkg/briaot` via `go generate ./pkg/briaot` (adds `pkg/briaot/bridb`).
  `SynthGoMod` carries the new requires automatically.

## 4. Tests

- [x] 4.1 `pkg/bri/db_test.go` (interpreter, in-memory SQLite): connect+query
...
- [x] 4.2 Dual-mode parity: a small bri.db app run interpreted AND compiled
  returns identical results (mirror `spikes/s45-bri-aot-docker/bri/parity/`).
- [x] 4.3 `pkg/briaot` drift test stays green (regenerated bridb committed).

## 5. Worked example

- [x] 5.1 A notes/todo CRUD wiring bri.db into bri.http (GET list, POST create,
  DELETE) persisting to SQLite — proving GET/POST/DELETE persist, both modes.

## 6. Gates

- [x] 6.1 `go build ./... && go vet ./... && gofmt -l pkg cmd conformance
