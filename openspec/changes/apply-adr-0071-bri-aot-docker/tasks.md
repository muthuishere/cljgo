...
- [ ] 1.1 Make `core/bri/*.cljg` resolve + emit at `cljgo build` the way
  `core/async.cljg` does (they are already embedded via `core/bri.go`). A user
  `(require '[bri.http])` must not error at compile.
- [ ] 1.2 Register the `pkg/bri` host shims (`-serve`/`-request`/`-json-*`/
...
- [ ] 1.3 Emitted `func main()` invokes the app `-main`. `cljgo build` on a bri
  hello-world → a static binary that serves `GET /` (text) + `GET /api/hello`
  (JSON). `CGO_ENABLED=0`; `ldd`/`otool -L` scratch-clean. Gates green.

## 2. Spike proof (s45)

- [ ] 2.1 `spikes/s45-bri-aot-docker/bri/`: the bri hello app (src + build) that
  compiles to a static binary and serves; record binary size + startup.

## 3. Dual-mode parity harness

- [ ] 3.1 A bri behavior suite runs the SAME app interpreted (`cljgo dev` / the
  ...
  (bad param → 400). A divergence fails. Gates green.
