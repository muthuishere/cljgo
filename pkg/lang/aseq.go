...
// aseqEquals is the Java ASeq.equals analog: same shape as aseqEquiv,
// but elements are compared with Equals (type-strict), per
// clojure.lang.ASeq.equals using Util.equals.
// (cljgo M0: was aliased to aseqEquiv; split with Equiv/Equals.)
func aseqEquals(a ASeq, obj any) bool {
	...
	ms := Seq(obj)
	for s := Seq(a); s != nil; s, ms = s.Next(), ms.Next() {
		if ms == nil || !Equals(s.First(), ms.First()) {
			return false
		}
	}
