...
type (
	...
	Node interface {
		...
		// entry on the heap for every successful lookup made every hash-map
		// READ allocate, language-wide, and ValAtDefault — the hot path under
		// every (get m k) — threw the key away immediately. EntryAt builds a
		// MapEntry from these values only when a caller actually wants one.
		find(shift uint, hash uint32, key any) (foundKey, foundVal any, ok bool)
		nodeSeq() ISeq
		iter() MapIterator
	}
...
func (m *PersistentHashMap) EntryAt(key any) IMapEntry {
	if m.root != nil {
		if k, v, ok := m.root.find(0, HashEq(key), key); ok {
...
func (m *PersistentHashMap) ValAtDefault(key, notFound any) any {
	if m.root != nil {
		if _, v, ok := m.root.find(0, HashEq(key), key); ok {
			return v
		}
	}
	return notFound
...
// Equals is the Java APersistentMap.equals analog (values compared
// type-strictly). (cljgo M0: added with the Equiv/Equals split.)
func (m *PersistentHashMap) Equals(o any) bool {
	return mapEquals(m, o)
}
...
func (b *BitmapIndexedNode) find(shift uint, hash uint32, key any) (any, any, bool) {
	bit := bitpos(hash, shift)
	if (b.bitmap & bit) == 0 {
...
func (n *ArrayNode) find(shift uint, hash uint32, key any) (any, any, bool) {
	idx := mask(hash, shift)
	node := n.array[idx]
	if node == nil {
		return nil, nil, false
...
func (n *HashCollisionNode) find(shift uint, hash uint32, key any) (any, any, bool) {
	idx := n.findIndex(key)
	if idx == -1 {
		return nil, nil, false
	}
