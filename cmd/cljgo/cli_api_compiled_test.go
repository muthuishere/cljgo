...
(defn -main [& args]
  ...
  ;; interpreted with a stubbed keychain.
  (let [a (api/api (str (first args) "/openapi.json") {:service "svc" :auth :token})]
    (println "ops" (count (api/operations a)))))
`
...
func TestBriCliApiCompiled(t *testing.T) {
	...
	if got := strings.TrimSpace(string(out)); !strings.Contains(got, "ops 2") {
		t.Fatalf("compiled bri.cli.api output =\n%q\nwant it to contain %q", got, "ops 2")
	}
}
