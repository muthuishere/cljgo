#!/usr/bin/env bash
# The web-framework benchmark: bri against the field, reproducibly.
#
# ONE container at a time — contention skews numbers, so nothing runs in
# parallel. For each runtime we build its image, cold-start it, warm it,
# load-test both routes with oha, sample peak RSS via `docker stats`, and
# record image size. Output is a markdown table at benchmark/web/results.md.
#
#   bash benchmark/web/run.sh                    # every runtime under compare/
#   DURATION=30s CONC=100 bash benchmark/web/run.sh
#
# Requires docker, oha and curl. The bri image is built FROM THIS CHECKOUT, so
# you are measuring your working tree, not a release.
#
# Every server answers the same two routes with byte-exact bodies:
#   GET /           -> "hello\n"
#   GET /api/hello  -> {"msg":"hello from <runtime>"}
#
# The cljgo entrant is the FLAGSHIP bri.http (bri/), not a raw net/http
# server — comparing a framework to a bare handler would flatter us.
set -uo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
ROOT="$HERE"                             # bri/ and compare/ live beside this script
REPO="$(cd "$HERE/../.." && pwd)"        # bri builds with the repo root as context
...
for e in "${ENTRIES[@]}"; do
  ...
  if [ "$name" = "bri" ]; then
    build_ok() { docker build -q -f "$dir/Dockerfile" -t "$img" "$REPO" >/dev/null 2>&1; }
  else
