...
description: "cljgo generate resource Note title:string scaffolds a migration, model, handlers, routes, and a green CRUD test over cljg.data.cast — and splices it into your app."
---
...
- **`src/app/db.cljg`** — the app's single datasource, created **once** and never clobbered. It wires the [cljg.data.cast](/cljgo/bri/db/) connection (SQLite by default, `:memory:` under the `:test` profile, Postgres via `APP_DB_URL`).
- **The resource** (`src/app/notes.cljg`) — a `coerce` fn (JSON body → typed columns), a model of parametrized `cljg.data.cast` calls, five handlers (`index`/`show`/`create`/`update-one`/`delete-one`), and a `routes` value. Every route is authenticated ([bri.core.security](/cljgo/bri/auth/)); `delete` is `admin-only`. **This file is yours** — edit it freely; the generator never rewrites an existing resource.
...
The model is the only place that touches `cljg.data.cast`, and every query is parametrized — the injection-safe seam kept in one file.
...
- [cljg.data.cast](/cljgo/bri/db/) — the data layer the generated model calls
