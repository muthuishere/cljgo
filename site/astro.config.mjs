		...
		starlight({
			...
			plugins: [
				starlightLlmsTxt({
					...
					details:
						'cljgo compiles Clojure to plain Go source and also runs it in a REPL from the same analyzer, so REPL and compiled binaries behave identically (dual-harness conformance-tested against real JVM Clojure). A compiled hello-world is a ~7.1 MB static binary that starts in ~5 ms (measured 2026-08-02; the cljgo tool itself is ~28.6 MB). It calls any Go package with zero bindings, ships single static binaries, includes the bri app framework (config, HTTP, HTML), a dependency/publishing story on the Go module ecosystem, and core.async. Performance is honestly reported — in the AOT-vs-AOT head-to-head against Glojure, cljgo wins the tree-recursion rows (tak, fib) and loses transducers and reduce, with four rows tied inside the run-to-run noise; the reduce loss is ~3.7x and is published alongside the wins. Errors are structured diagnostics with registered codes and `cljgo explain <code>` pages.',
				}),
			],
			customCss: [
